...
pub struct ProverKey<E: Engine> {
  pub pk_ee: ipa_pc::ProverKey<E>,
  pub S_repr: Vec<R1CSShapeSparkRepr<E>>,
...
pub struct BatchedRelaxedR1CSSNARK<E: Engine> {
  ...
  //vals_Az_Bz_Cz_W_E: Vec<[E::Scalar; 5]>,
  ...
  CommitmentKey<E>: CommitmentKeyExtTrait<E>,
{
  ...
  fn prove(
    ...
    let mut w_transcript = E::TE::new(b"WitnessOnly");
    ...
    // Compute eq(tau) for each instance
    let tau_witness = w_transcript.squeeze(b"t")?;
    ...
    let comms_Az_Bz_Cz: Vec<_> = comms_Az_Bz_Cz
      ...
      .collect();
    let comms_L_row_col: Vec<_> = comms_L_row_col
      .into_iter()
      .map(|comms| comms.map(|comm| comm.compress()))
      .collect();
  ...
  fn verify(&self, vk: &Self::VerifierKey, U: &[RelaxedR1CSInstance<E>]) -> Result<(), NovaError> {
    ...
    let mut transcript = E::TE::new(b"WitnessOnly");
  ...
  CommitmentKey<E>: CommitmentKeyExtTrait<E>,
{
  ...
  pub fn verify_untrusted(
    ...
    // absorb the claimed evaluations into the transcript
    self.data.evals_Az_Bz_Cz_at_tau.iter().for_each(|evals| {
      transcript.absorb(b"e", &evals.as_slice());
    });
    ...
    let evals_Mz: Vec<_> = zip_with!(
      iter,
      (comms_Az_Bz_Cz, self.data.evals_Az_Bz_Cz_at_tau),
    ...
    for evals in evals_vec.iter() {
      u_transcript.absorb(b"e", &evals.as_slice()); // comm_vec is already in the transcript
    }
    ...
    E::Scalar: PrimeField,
  {
    ...
    let inner_sc_inst = zip_with!(
      par_iter,
      (pk.S_repr, evals_Mz, data.polys_L_row_col),
    ...
    let witness_sc_inst = zip_with!(par_iter, (data.polys_W, data.S), |poly_W, S| {
      WitnessBoundSumcheck::<E>::new(data.tau, poly_W.clone(), S.num_vars)
    })
    .collect::<Vec<_>>();
